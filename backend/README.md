...
- `health`, `readiness` и `requestId`-tracking в error responses
...
Минимальный operational baseline теперь включает:

- `GET /api/health` для liveness
- `GET /api/health/ready` для readiness с проверкой database connectivity
- автоматический `x-request-id` в каждом ответе
- structured request logs с `method`, `path`, `statusCode`, `durationMs`, `actor`
- standardized error body с `requestId`, чтобы ошибку можно было быстро найти в логах
